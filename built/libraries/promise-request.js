...
class PromiseRequest {
    constructor(url, options) {
        this.cfbypass = new cfbypass_1.default((process.env.IS_PYTHON3?.toLowerCase() == "true" ? true : false) || false);
        this.url = url;
            ...
            try {
                if (this.options.stream) {
                    ...
                    if (options.bypassWaf) {
                        ...
                        if (canFetch) {
                            const fetchKey = axios_1.default.get(`${process.env.NINEANIME || "https://9anime.pl"}/?__jscheck=${token}`, {
                                headers: {
