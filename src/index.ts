...
export let unresolvedEvents: any[] = [];
emitter.onAny((event, ...args) => {
    const promise = EventEmitter2.once(emitter, event).then(
        (data) => {
            unresolvedEvents = unresolvedEvents.filter((e) => e !== promise);
        },
        (err) => {
            unresolvedEvents = unresolvedEvents.filter((e) => e !== promise);
        }
